...
function getRefFromUrl(){
  try{
    ...
    if (ref) {
      localStorage.setItem("chronos_ref", ref);
      return ref;
    }
  }catch(e){}
  return (localStorage.getItem("chronos_ref") || "").trim();
}
...
async function copyText(text){
  try{
    ...
    // fallback (older Android WebView)
    try{
...
function updateReferralUI(){
  const codeEl = $("#refCode");
  const linkEl = $("#refLink");
  const plusEl = $("#refPlusCount");
  const premEl = $("#refPremiumCount");
  if (!codeEl || !linkEl) return;

  if (!state.me || !state.me.referral_code){
    codeEl.value = "—";
    linkEl.value = "—";
    if (plusEl) plusEl.textContent = "0";
    if (premEl) premEl.textContent = "0";
    setRefMsg(state.token ? "No se encontró referral_code en tu perfil." : "Inicia sesión para ver tu enlace de referido.");
    return;
  }

  const code = String(state.me.referral_code);
  const link = `${window.location.origin}/?ref=${encodeURIComponent(code)}`;
  codeEl.value = code;
  linkEl.value = link;

  if (plusEl) plusEl.textContent = String(state.me.referrals_plus ?? 0);
  if (premEl) premEl.textContent = String(state.me.referrals_premium ?? 0);
  setRefMsg("");
}
...
async function doRegister() {
  ...
  try {
    await api("/auth/register", { method:"POST", auth:false, body:(() => { const ref = getRefFromUrl(); return ref ? { email, password, referral_code: ref } : { email, password }; })()});
...
async function doLogin() {
  ...
  try {
    const data = await api("/auth/login", { method:"POST", auth:false, body:(() => { const ref = getRefFromUrl(); return ref ? { email, password, referral_code: ref } : { email, password }; })()});
...
function bindUI() {
  ...
  const btnCRC = $("#btnCopyRefCode");
  if (btnCRC) btnCRC.addEventListener("click", async () => {
    setRefMsg("");
    const code = ($("#refCode") && $("#refCode").value || "").trim();
    if (!code || code === "—") return setRefMsg("No hay código para copiar.");
    const ok = await copyText(code);
    setRefMsg(ok ? "Código copiado." : "No pude copiar el código.");
  });
  const btnCRL = $("#btnCopyRefLink");
  if (btnCRL) btnCRL.addEventListener("click", async () => {
    setRefMsg("");
    const link = ($("#refLink") && $("#refLink").value || "").trim();
    if (!link || link === "—") return setRefMsg("No hay enlace para copiar.");
    const ok = await copyText(link);
    setRefMsg(ok ? "Enlace copiado." : "No pude copiar el enlace.");
  });
...
async function boot() {
  bindUI();
  // Guarda ?ref=XXXX en localStorage para usarlo en el registro
  getRefFromUrl();
  showView("dashboard");
  await loadMe();
  updateReferralUI();
